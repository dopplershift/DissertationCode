...
from disser.units import to_dB, to_dBz
...
for ind, t in enumerate(temps):
    ...
    zdr = z - to_dBz(scatt.get_reflectivity_factor(dist, polar='v'))
    atten = (scatt.get_attenuation(dist, polar='h').rescale(1/pq.km) *
            db_factor)
    diff_atten = (atten -
            scatt.get_attenuation(dist, polar='v').rescale(1/pq.km) * db_factor)
    kdp = (scatt.get_propagation_wavenumber(dist, polar='h') -
            scatt.get_propagation_wavenumber(dist, polar='v'))
    delta = scatt.get_backscatter_differential_phase(dist)
...
ax[0,0].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[0,0].set_ylabel('Backscatter Cross-Section')
ax[0,0].legend(loc='upper left')
ax[0,0].grid()
ax[0,0].set_xlim(0, d_plot.max().magnitude)

ax[0,1].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[0,1].set_ylabel('Horizontal Extinction Cross-Section')
ax[0,1].grid()

ax[0,2].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[0,2].set_ylabel('Differential Backscatter Phase')
ax[0,2].grid()

ax[1,0].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[1,0].set_ylabel('Differential Backscatter Cross-Section')
ax[1,0].grid()

ax[1,1].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[1,1].set_ylabel('Differential Extinction Cross-Section')
ax[1,1].grid()

ax[1,2].set_xlabel('Diameter (%s)' % d_plot.dimensionality)
ax[1,2].set_ylabel('Differential Wavenumber')
ax[1,2].grid()

ax2[0,0].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[0,0].set_ylabel('Reflectivity Factor')
ax2[0,0].grid()
ax2[0,0].set_xlim(0, 35)

ax2[0,1].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[0,1].set_ylabel('Attenuation')
ax2[0,1].grid()

ax2[0,2].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[0,2].set_ylabel('Differential Backscatter Phase')
ax2[0,2].grid()

ax2[1,0].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[1,0].set_ylabel('Differential Reflectivity')
#ax2[1,0].set_ylim(-6,6)
ax2[1,0].grid()

ax2[1,1].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[1,1].set_ylabel('Differential Attenuation')
ax2[1,1].grid()

ax2[1,2].set_xlabel('Rain Water (%s)' % qr.dimensionality)
ax2[1,2].set_ylabel('KDP')
ax2[1,2].grid()

ax3[0].set_xlabel('KDP (%s)' % kdp.dimensionality)
ax3[0].set_ylabel('Attenuation')
ax3[0].grid()
ax3[0].set_xlim(0,3)

ax3[1].set_xlabel('KDP (%s)' % kdp.dimensionality)
